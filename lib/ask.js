...
/**
 * @param {{ session?: string, from?: string, text?: string, interactiveId?: string, location?: {latitude:number,longitude:number}, type?: string, image?: object, channel?: string, hash?: string }} input
 */
async function ask(input = {}) {
  ...
  const payloads = await engine.handle({
    ...
    type: input.type,
    channel: input.channel || (input.from ? 'whatsapp' : 'api'),
    image: input.image
  });
  const parsed = engine.lastParse || { intent: null, places: [], via: 'api' };
