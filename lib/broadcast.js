...
function recordOptIn(hash, capability, method = 'keyword', source = 'whatsapp') {
  const row = { hash, capability, at: new Date().toISOString(), method, source };
  optIns.push(row);
  persist.saveConsent(row).catch(e => console.error('neon consent', e.message || e));
  return row;
}
...
async function ready() {
  ...
  booted = (async () => {
    const [rows, sends] = await Promise.all([persist.loadConsent(), persist.loadSends()]);
    if (rows.length && !optIns.length) {
    ...
    if (sends.length && !sendLog.length) {
...
function execute(planned, buildPayload) {
  ...
  planned.recipients.slice(0, planned.estimated_messages).forEach(hash => {
    ...
    const row = { hash, template: planned.template, at: new Date().toISOString() };
    sendLog.push(row);
    persist.saveSend(row).catch(e => console.error('neon send', e.message || e));
  });
...
module.exports = { CONFIG, TIERS, SEGMENTS, recordOptIn, optInProof, plan, execute, ledger, optIns, sendLog, ready };
