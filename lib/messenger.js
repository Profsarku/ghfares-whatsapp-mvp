...
/* ═══════════ PERSISTENT MENU ═══════════
   Messenger's real advantage over WhatsApp: a menu that is always visible,
   rather than a slash command the user must know to type. 3 top-level items,
   submenus allowed. POST to /me/messenger_profile. */

function messengerProfile(commands) {
  return {
    ...
    persistent_menu: [{
      ...
      call_to_actions: [
        ...
        {
          type: 'nested', title: 'Check something',
          call_to_actions: [
            { type: 'postback', title: 'Fares from here', payload: 'loc' },
            { type: 'postback', title: 'Fuel prices', payload: 'ask:fuel' },
            { type: 'postback', title: 'Road conditions', payload: 'ask:road' },
            { type: 'postback', title: 'How to talk to me', payload: 'help' }
          ]
        },
        {
          type: 'nested', title: 'Add-ons',
          call_to_actions: [
            { type: 'postback', title: 'See all add-ons', payload: 'addon:menu' },
            { type: 'postback', title: 'Add fuel watch', payload: 'addon:add:FUEL' },
            { type: 'postback', title: 'Add road alerts', payload: 'addon:add:ROADS' },
            { type: 'postback', title: 'Save my route', payload: 'addon:add:MY ROUTE' },
            { type: 'postback', title: 'My add-ons', payload: 'addon:list' }
          ]
        }
      ]
    }]
