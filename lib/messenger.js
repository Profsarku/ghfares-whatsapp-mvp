...
function fromWhatsApp(p, psid) {
  ...
  if (i.type === 'location_request_message') {
    return [text(psid, i.body.text, [
      { location: true },
      { id: 'place:type', title: "I'll type it" }
    ])];
  }
...
function messengerProfile() {
  return {
    ...
    greeting: [{
      locale: 'default',
      text: 'Help others never get stranded, lost, or run out of transport money. Ask a fare, send a road photo, then say where you are.'
    }],
    ice_breakers: [{
      locale: 'default',
      call_to_actions: [
