...
function startMenu(to, hash) {
  ...
  return wa.list(
    ...
    'GH Fares',
    'Or type: kaneshie to bubuashie · /fare · /roads'
  );
}
...
async function handleRoadPhoto(from, hash, image, s) {
  ...
  if (!hasBytes) {
    return wa.text(from, 'I could not pull that photo from WhatsApp. Send it again from the camera, and add a caption like *tema motorway blocked* if you can.');
  }
...
async function handleRoadTextReport(from, hash, c, s, text) {
  ...
  if (vague || !named) {
    ...
    return wa.buttons(from,
      `Send a photo${hint} from WhatsApp, or say what you see — like *pothole on kaneshie to kasoa* or *tema motorway blocked*.`,
      [{ id: 'addon:add:ROADS', title: 'Add road alerts' }, { id: 'menu', title: 'Main menu' }]);
  }
...
async function dispatch({ from, hash, text, interactiveId, location, type, s, image }) {
  ...
  if (location) {
    ...
    if (!near.station || near.too_far) {
      ...
      const hint = near.station && km != null
        ...
        : '';
      return [wa.text(from, hint + 'Share your location when you are at a Ghana station, or send the name — like *kaneshie*.')];
    }
  ...
  if (interactiveId) {
    ...
    if (p[0] === 'ask') {
      ...
      return [wa.text(from, 'Send it like *kaneshie to bubuashie*.')];
    }
    if (p[0] === 'fuelstation') {
