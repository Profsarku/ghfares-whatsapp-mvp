...
async function dispatch({ from, hash, text, interactiveId, location, type, s, image }) {
  ...
  if (text && (/^delete my data$/i.test(String(text).trim()) || /^forget me$/i.test(String(text).trim()))) {
    caps.forget(hash);
    return [wa.text(from, 'Your GH Fares data for this chat is deleted — add-ons, saved station, route, country, and road photos. Message history inside WhatsApp or Messenger is still held by Meta. Send *hi* if you want to start again.')];
  }
  ...
  /* location share → road-photo place first (images have no GPS), then nearest station */
  if (location) {
