...
function handle({ from, hash, text, interactiveId, location, type }) {
  ...
  if (text) {
    if (/^delete my data$/i.test(String(text).trim()) || /^forget me$/i.test(String(text).trim())) {
      caps.forget(hash);
      return [wa.text(from, 'Your GH Fares data for this chat is deleted — add-ons, saved station and route. Message history inside WhatsApp or Messenger is still held by Meta. Send *hi* if you want to start again.')];
    }
