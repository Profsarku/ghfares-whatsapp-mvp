...
/* ─────────────── classifier ───────────────
   Regex fast-path first. An open-source model (Ollama / Llama) runs only
   on a miss, or when the rider is talking about add-ons in their own
   words. It may only return {intent, entities} — never a price. Every
...
function classifyRegex(raw) {
  ...
  const out = { intent: null, places: [], compare: false, scope: null, past: false, amount: null, arg: undefined, raw, via: 'regex' };
...
async function classify(raw) {
  ...
  const ambiguousQuery = out.intent && nlu.looksLikeAddonControl(raw)
    && ['fuel', 'road', 'queue', 'fare', 'cheapest'].includes(out.intent);
  if (out.intent && !ambiguousQuery) return out;
  ...
  if (!out.intent) return heuristicIntent(out);
  return out;
}
...
const api = {
  ...
  async reportRoadPhoto({ road, condition, caption, mime, bytes, wa_media_id, hash, where } = {}) {
    ...
    const logged = await this.reportRoad(name, cond, {
      kind: cond,
      where: where || caption || 'WhatsApp photo',
      hash
    });
    return {
      photo,
      ...
      neon: persist.enabled('report_road_condition')
    };
  },
...
let crowdReady = null;
function ready() {
  if (!crowdReady) crowdReady = persist.overlayCrowd(core);
  return crowdReady;
}

module.exports = { classify, classifyRegex, api, PLACES, nlu, ready };
