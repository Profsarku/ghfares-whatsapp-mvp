...
async function loadUsers() {
  ...
  return (await safe('load users', () => neon.sql('users')`
    SELECT hash, caps, route, station, country, context, pending, seen FROM users
  `)) || [];
}
...
async function saveUser(s) {
  ...
  await safe('save user', () => neon.sql('users')`
    INSERT INTO users (hash, caps, route, station, country, context, pending, seen)
    VALUES (${s.hash}, ${caps}, ${s.route || null}, ${s.station || null}, ${s.country || null},
...
async function saveRoadPhotoKind(photo_id, kind) {
  if (!neon.enabled('road_photos') || !photo_id || !kind) return;
  await safe('save road photo kind', () => neon.sql('road_photos')`
    UPDATE photos SET photo_kind = ${kind} WHERE id = ${photo_id}
  `);
}

async function saveRoadCondition({ road, condition, kind, where_text, delay, hash, status, confirmations, photo_id, check_accurate, check_seen, check_nlu, photo_kind }) {
  if (!neon.enabled('report_road_condition')) return;
  ...
  await safe('save road condition', async () => {
    ...
    const inserted = await q`
      INSERT INTO reports (road_key, road, condition, kind, where_text, delay, hash, photo_id, check_accurate, check_seen, check_nlu, photo_kind)
      VALUES (${road_key}, ${road}, ${cond}, ${kind || null}, ${where_text || null},
              ${delay || null}, ${hash || null}, ${photo_id || null}, ${check_accurate ?? null},
              ${check_seen || null}, ${check_nlu || null}, ${photo_kind || null})
      RETURNING id
    `;
    report_id = inserted[0] && inserted[0].id;
  ...
  return { road_key, road, condition: cond, status: st, id: report_id, photo_id: photo_id || null };
}

async function saveRoadPhoto({ road, condition, caption, mime, bytes, wa_media_id, hash }) {
  if (!neon.enabled('road_photos')) return null;
  ...
  const row = await safe('save road photo', async () => {
    const q = neon.sql('road_photos');
    const inserted = await q`
      INSERT INTO photos (hash, road_key, road, condition, caption, mime, bytes, wa_media_id)
      VALUES (${hash || ''}, ${road_key}, ${roadName}, ${cond}, ${caption || null}, ${type},
              decode(${hex}, 'hex'), ${wa_media_id || null})
      RETURNING id
    `;
...
async function saveRoadPhotoCheck(id, check) {
  if (!neon.enabled('road_photos') || !id || !check) return;
  await safe('save road photo check', () => neon.sql('road_photos')`
    UPDATE photos SET
      check_accurate = ${check.accurate ?? null},
      check_seen = ${check.seen || null},
      check_nlu = ${check.nlu || null},
      check_reason = ${check.reason || null},
      photo_kind = ${check.kind || null}
    WHERE id = ${id}
  `);
}

async function linkRoadPhotoReport(photo_id, report_id) {
  if (!neon.enabled('road_photos') || !photo_id || !report_id) return;
  await safe('link road photo report', () => neon.sql('road_photos')`
    UPDATE photos SET report_id = ${report_id} WHERE id = ${photo_id}
  `);
}

async function saveRoadPhotoPlace(photo_id, { lat, lng, where_text, contractor } = {}) {
  if (!neon.enabled('road_photos') || !photo_id) return;
  await safe('save road photo place', () => neon.sql('road_photos')`
    UPDATE photos SET
      lat = COALESCE(${lat ?? null}, lat),
      lng = COALESCE(${lng ?? null}, lng),
      where_text = COALESCE(${where_text || null}, where_text),
      contractor = COALESCE(${contractor || null}, contractor)
    WHERE id = ${photo_id}
  `);
}

async function saveRoadReportPlace(report_id, { lat, lng, where_text, contractor, photo_kind } = {}) {
  if (!neon.enabled('report_road_condition') || !report_id) return;
  await safe('save road report place', () => neon.sql('report_road_condition')`
    UPDATE reports SET
      lat = COALESCE(${lat ?? null}, lat),
      lng = COALESCE(${lng ?? null}, lng),
      where_text = COALESCE(${where_text || null}, where_text),
      contractor = COALESCE(${contractor || null}, contractor),
      photo_kind = COALESCE(${photo_kind || null}, photo_kind)
    WHERE id = ${report_id}
  `);
}

async function loadBadRoads() {
  if (!neon.enabled('report_road_condition')) return [];
  const q = neon.sql('report_road_condition');
  const rows = await safe('load bad roads', () => q`
    SELECT id, road, condition, kind, where_text, contractor, photo_id, lat, lng, photo_kind, at
    FROM reports
    WHERE condition IS NOT NULL AND lower(condition) NOT IN ('clear', 'cleared')
      AND (photo_kind IS NULL OR photo_kind IN ('road_condition', 'accident'))
    ORDER BY at DESC
    LIMIT 80
  `);
  if (rows) return rows;
  const fallback = await safe('load bad roads fallback', () => q`
    SELECT id, road, condition, kind, where_text, photo_id, at
    FROM reports
    WHERE condition IS NOT NULL AND lower(condition) NOT IN ('clear', 'cleared')
    ORDER BY at DESC
    LIMIT 80
  `);
  return (fallback || []).map(r => ({ ...r, contractor: '', lat: null, lng: null, photo_kind: null }));
}

async function loadRoadPhotoHistory(hash) {
  if (!hash || !neon.enabled('road_photos')) return [];
  const rows = await safe('load road photo history', () => neon.sql('road_photos')`
    SELECT id, road, condition, caption, mime, check_accurate, check_seen, check_nlu, report_id, at, where_text, photo_kind
    FROM photos WHERE hash = ${hash} ORDER BY at DESC LIMIT 20
  `);
  return rows || [];
}
...
async function forget(hash) {
  ...
  await safe('forget', async () => {
    ...
    if (neon.enabled('report_road_condition')) {
      ...
      await neon.sql('report_road_condition')`DELETE FROM reports WHERE hash = ${hash}`;
    }
    if (neon.enabled('road_photos')) {
      await neon.sql('road_photos')`DELETE FROM photos WHERE hash = ${hash}`;
    }
  });
}
...
async function seedCountries(rows) {
  if (!neon.enabled('countries')) return;
  await safe('seed countries', async () => {
    const q = neon.sql('countries');
    for (const c of rows || []) {
      const bbox = c.bbox || [];
      await q`
        INSERT INTO countries (iso, name, aliases, live, data_db, lat_min, lat_max, lng_min, lng_max)
        VALUES (${c.iso}, ${c.name}, ${c.aliases || []}, ${!!c.live}, ${c.data_db || null},
...
async function loadCountries() {
  if (!neon.enabled('countries')) return [];
  return (await safe('load countries', () => neon.sql('countries')`
    SELECT iso, name, aliases, live, data_db, lat_min, lat_max, lng_min, lng_max
